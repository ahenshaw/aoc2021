...
fn get_stats(bits: &Input) -> Vec<Counter<char>> {
    ...
    .map(|i| {
        ...
        bits.iter().for_each(|row| {count[&row.chars().nth(i).unwrap()] += 1;});
        count
    }).collect()
}
...
pub fn part2(input: &Input) -> usize {
    ...
    for i in 0..n {
        while oxygen.len() > 1 {
            ...
            oxygen = oxygen.iter().filter(|bits| bits.chars().nth(i).unwrap() == desired).map(|x| x.clone()).collect();
        }
        while co2.len() > 1 {
            let desired = get_stats(&co2)[i].most_common()[1].0;
            co2 = co2.iter().filter(|bits| bits.chars().nth(i).unwrap() == desired).map(|x| x.clone()).collect();
        }
    }
